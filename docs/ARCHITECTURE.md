...
`design`은 섹션별 패턴·순서·숨김·장식·밀도, `footer`는 사용자 하단입력 원고를 담는다. 기존 JSON에 이 필드가 없으면 템플릿별 기본 패턴을 적용한다. 사이트 원본은 계속 `sites/<id>/site.json`이며 레거시 templateContent 예시를 DIY로 가져올 때 기본 원고에 해소하고 override를 제거한다.
...
미리보기 iframe은 처음에만 srcdoc를 설정하고 이후 공통 렌더러의 결과를 DOM으로 반영한다. 섹션 id로 노드를 재사용하고 잠시 숨긴 섹션을 메모리에 보관하여 작성 중 스크롤, FAQ 펼침, 상담 체험 상태를 유지한다. 새 상담 폼에만 멱등 초기화 함수를 적용한다. 다른 작성 블록으로 포커스를 옮길 때만 해당 미리보기 섹션으로 이동한다.

스타일에서 선택한 섹션만 작성 순서대로 노출한다. 숨긴 원고도 JSON 안에 보존하며 표시 여부에 따라 필수 내용 검증을 적용한다. 형식·글자 수·안전성 검증과 항상 표시되는 하단 연락처·고지는 유지한다. `src/render/fonts.ts`가 여섯 제목 서체의 이름, CSS, 글꼴 스택과 공식 라이선스를 한곳에서 관리한다.
