...
for(let attempt=1;attempt<=30;attempt++) {
  try {
    for(const theme of themes) for(const palette of palettes) {
      ...
      assert.ok(html.includes('data-design-version="calm-trust-v5"'),'new design not yet live');
...
const summary=`Calm Trust v5 live verification passed: 5 layouts × 6 palettes, gallery, noindex, company name, demo-only form, completion notice and privacy.\n${origin}/templates\n`;
console.log(summary);
