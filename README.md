...
- `/studio`: 검증된 커스텀 제작본을 선택하고 색상 팔레트, 담당자 정보, 연락처, 프로필 사진, 좌상단 로고, 푸터 로고를 바꾸는 설정 화면입니다. 현재는 김경현 템플릿과 같은 구조의 김대경 포레스트 변형을 제공합니다. 입력값은 즉시 실제 렌더러 미리보기에 반영되며 PC/모바일 미리보기를 유지합니다. 워터마크는 해온 기본 자산을 고정 사용합니다.
