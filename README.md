...
- `/studio`: 현재 고객 페이지 디자인은 고정하고 색상 팔레트, 담당자 정보, 연락처, 프로필 사진, 좌상단 로고, 푸터 로고만 바꾸는 설정 화면입니다. 입력값은 즉시 실제 렌더러 미리보기에 반영되며 PC/모바일 미리보기를 유지합니다. 워터마크는 해온 기본 자산을 고정 사용합니다.
- `/proposal`: 보험사·GA·지사에 설명할 제공 범위와 운영 절차. 고객 상담 페이지와 구분합니다.
- 설정 화면의 `site.json 저장`은 현재 값을 JSON 파일 하나로 내려받습니다. 게시나 서버 전송 기능은 포함하지 않습니다.
