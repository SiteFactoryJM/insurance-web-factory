...
async function directLinks(page){
 ...
 await expect(hero.locator('[data-contact-link="instagram"]')).toHaveAttribute('href','https://www.instagram.com/91ybok');
 await expect(hero.locator('[data-contact-link="instagram"]')).toHaveAttribute('target','_blank');
...
test('phone and open chat links also work without JavaScript',async({browser})=>{
 ...
 try{const page=await context.newPage();await page.goto('/');await directLinks(page);
  // 자바스크립트 없이도 번호와 전화·카카오톡·인스타그램 링크가 그대로 보인다.
  await expect(page.locator('#contact')).toContainText('010-4187-7511');
  await expect(page.locator('#contact [data-contact-link]')).toHaveCount(3);}
...
test('keyboard skip, FAQ, mobile menu and floating contact links are usable',async({page})=>{
 ...
 const dock=page.locator('[data-floating-contact]');await expect(dock).toBeVisible();
 await expect(dock.locator('[data-contact-link]')).toHaveCount(3);
 await page.locator('#about').scrollIntoViewIfNeeded();await expect(dock).toBeVisible();
 await page.locator('#contact').scrollIntoViewIfNeeded();await expect(dock).toBeVisible();
});
...
test('consumer interactions never submit customer data or store it',async({page})=>{
 ...
 await page.goto('/');await page.locator('#faq summary').first().click();await page.locator('.desktop-nav a[href="#specialties"]').click();
