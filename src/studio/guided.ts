...
document.addEventListener('click', async event => {
  ...
  try {
    ...
    if (target.dataset.template) {
      ...
      message(`${TEMPLATE_META[site.template].name} 배치를 적용했습니다. 첫 화면·섹션 순서·표현 방식이 바뀌고 문구·색상·실제 정보는 유지됩니다.`);
    }
