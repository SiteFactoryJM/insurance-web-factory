import { TEMPLATE_IDS, type SiteConfig, type PaletteId, type HeadingFont, type HeroBrandLayout, type TemplateId } from '../types.js';
...
const BRAND_CHOICES: {id:HeroBrandLayout;name:string;description:string;tagline:string;subline:string}[] = [
  { id:'soft-panel', name:'소프트 브랜드 패널', description:'사람을 먼저 생각하는 보험의 기준, 해온', tagline:'사람을 먼저 생각하는 보험의 기준, 해온', subline:'LIFE INSURANCE FOR A BRIGHTER TOMORROW' },
  { id:'gold-wave', name:'골드 웨이브 배너', description:'당신의 오늘이 더 나은 내일이 되도록', tagline:'당신의 오늘이 더 나은 내일이 되도록', subline:'LIFE, ALWAYS WITH YOU' },
  { id:'watermark', name:'워터마크 웨이브', description:'큰 로고와 HAEON · YOU FIRST를 은은하게 배경처럼 배치', tagline:'', subline:'' },
];
...
function siteForPreview(): SiteConfig {
  const out = structuredClone(site);
  // 새 작업은 실제 고객 로고를 자동 저장하지 않지만, 로고 연출 3가지는 바로 비교할 수 있도록
  // 미리보기에서만 예시 로고를 사용합니다. JSON/PDF 내보내기에는 이 임시 로고가 들어가지 않습니다.
  if (!out.agent.logoImage && example.agent.logoImage) out.agent.logoImage = example.agent.logoImage;
  if (!out.agent.logoMarkImage && example.agent.logoMarkImage) out.agent.logoMarkImage = example.agent.logoMarkImage;
  return out;
}
function preview(section?: string): void {
  ...
  previewTimer = window.setTimeout(() => {
    ...
    frame.srcdoc = renderSitePage(siteForPreview(), new Request(`${location.origin}/`), {studioPreview: true});
  }, 160);
}
...
function renderPanel(): void {
  ...
  if (stage === 0) {
    ...
    panel.innerHTML = `<p class="g-kicker">01 · 디자인과 문구</p><h2 tabindex="-1">구성과 색상을 고르고, 문구를 고르세요.</h2><p class="g-muted">화면 구성을 고르면 첫 화면과 순서, 보여 주는 방식이 바뀝니다. 색상과 이미 고른 문구, 담당자 정보는 그대로 남습니다.</p><h3>화면 구성 5가지</h3><div class="g-layout-grid">${TEMPLATE_IDS.map(id => `<button type="button" class="g-layout-choice" data-template="${id}" aria-pressed="${site.template === id}"><span class="g-layout-diagram g-layout-${id}" aria-hidden="true"><i></i><i></i><i></i></span><strong>${e(TEMPLATE_META[id].name)}</strong><small>${e(TEMPLATE_META[id].description)}</small></button>`).join('')}</div><h3>색상 6가지</h3><div class="g-palette-grid">${Object.entries(PALETTES).map(([id,p]) => `<button type="button" class="g-palette-choice" data-palette="${id}" aria-pressed="${site.palette === id}"><span class="g-palette-swatch" style="background:${p.accent}" aria-hidden="true"></span><span>${e(p.name)}</span></button>`).join('')}</div><label class="g-field"><span>제목 글꼴</span><select id="guided-font"><option value="noto-sans-kr"${site.headingFont === 'noto-sans-kr' ? ' selected' : ''}>기본 고딕</option><option value="noto-serif-kr"${site.headingFont === 'noto-serif-kr' ? ' selected' : ''}>차분한 명조</option><option value="pretendard"${site.headingFont === 'pretendard' ? ' selected' : ''}>단정한 고딕</option>${!['pretendard','noto-serif-kr','noto-sans-kr'].includes(site.headingFont) ? `<option value="${e(site.headingFont)}" selected>불러온 글꼴 그대로</option>` : ''}</select></label><details class="g-advanced-note"><summary>상담 목적에 맞춘 추천으로 한 번에 채우기</summary><p>추천을 고르면 구성·색상과 여러 문구가 한꺼번에 바뀝니다. 이름·연락처는 바뀌지 않습니다.</p><div class="g-purpose-grid">${library.presets.map(p => `<button type="button" class="g-purpose" data-purpose="${p.id}" aria-pressed="${purpose === p.id}"><strong>${e(p.label)}</strong><span>${e(p.audience)}</span></button>`).join('')}</div></details><div class="g-section"><h3>문구 고르기</h3><p class="g-muted">첫 화면부터 맨 아래 안내까지 ${Object.values(groups).length}개 묶음의 문장을 그대로 보고 고릅니다. 고른 문구는 구성이나 색상을 바꿔도 남습니다.</p><div class="g-copy-search"><label class="g-field"><span>바꿀 부분</span><select id="copy-group">${Object.entries(groups).map(([id,label]) => `<option value="${id}"${id === group ? ' selected' : ''}>${e(label)}</option>`).join('')}</select></label><label class="g-field"><span>문구 찾기</span><input id="copy-search" type="search" value="${e(copySearch)}" placeholder="예: 가족, 갱신, 상담 시간"></label><p id="choice-help" class="g-muted"></p></div><div id="copy-options" class="g-choice-list"></div><button type="button" class="g-btn g-btn-primary" data-action="apply-copies"${['services','faqs'].includes(group) ? '' : ' hidden'}>고른 문구 적용</button></div><p class="g-note">추천 문구는 그대로 쓰라는 뜻이 아닙니다. 실제로 상담하는 범위와 소속 기준에 맞는 문장만 남겨 주세요.</p>`;
    renderOptions(copySearch);
  } else if (stage === 1) {
    panel.innerHTML = `<p class="g-kicker">02 · 실제 정보</p><h2 tabindex="-1">실제 정보를 확인해 주세요.</h2><p class="g-muted">고객에게 보이는 정보만 입력합니다. 새로 시작할 때 예시 담당자 정보가 저절로 들어가지는 않습니다.</p><button class="g-btn" type="button" data-action="example-profile">예시 담당자 정보로 채워 보기</button><div class="g-row">${textField('담당자 이름','name',site.agent.name,60,'홍길동')}${textField('직함','title',site.agent.title,80,'보험설계사')}</div>${textField('소속','company',site.agent.company,120,'실제 보험사·GA·지사명')}<div class="g-row">${textField('전화번호','phone',site.contact.phone,32,'010-0000-0000','tel')}${textField('상담 시간','hours',site.contact.availableHours,100,'평일 09:00–18:00')}</div><div class="g-row">${textField('이메일 · 선택','email',site.contact.email,254,'name@example.com','email')}${textField('팩스 · 선택','fax',site.contact.fax,32,'02-0000-0000','tel')}</div>${textField('카카오톡 오픈채팅 주소','kakao',site.contact.kakaoUrl,2048,'https://open.kakao.com/o/초대코드','url')}${textField('인스타그램 주소 · 선택','instagram',site.contact.instagramUrl,2048,'https://www.instagram.com/사용자명','url')}${textField('쓰고 싶은 주소 · 선택','requestedDomain',requestedDomain,253,'agent.example.com')}<p class="g-muted">쓰고 싶은 주소는 적어 두는 칸입니다. 여기에 적는다고 주소가 만들어지지는 않습니다.</p><label class="g-field"><span>프로필 사진</span><input id="guided-photo" type="file" accept="image/jpeg,image/png,image/webp"><small>JPG·PNG·WebP, 3MB 이하. 직접 찍었거나 사용해도 되는 사진만 올려 주세요.</small></label><div class="g-row"><label class="g-field"><span>메인 로고 이미지 · 선택</span><input id="guided-logo" type="file" accept="image/jpeg,image/png,image/webp"><small>가로형 로고 권장. JPG·PNG·WebP, 3MB 이하. 올리면 미리보기에 바로 표시됩니다.</small></label><label class="g-field"><span>상단·푸터 로고 이미지 · 선택</span><input id="guided-logo-mark" type="file" accept="image/jpeg,image/png,image/webp"><small>심볼·세로형 로고 권장. 비워 두면 메인 로고를 함께 사용합니다.</small></label></div><section class="g-brand-section" aria-labelledby="brand-style-title"><h3 id="brand-style-title">메인 로고 연출 샘플 3가지</h3><p class="g-muted">샘플을 누르면 오른쪽 미리보기에 즉시 반영됩니다. 로고를 아직 올리지 않았다면 미리보기에서만 HAEON 예시 로고로 형태를 보여주며, 내보내는 JSON에는 선택한 연출·문구만 저장됩니다.</p><div class="g-brand-grid">${BRAND_CHOICES.map(choice => `<button type="button" class="g-brand-choice" data-brand-layout="${choice.id}" aria-pressed="${(site.hero.brandLayout || 'soft-panel') === choice.id}"><span class="g-brand-diagram g-brand-${choice.id}" aria-hidden="true"><b>HAEON</b><i></i><em>${e(choice.tagline || 'YOU FIRST')}</em></span><strong>${e(choice.name)}</strong><small>${e(choice.description)}</small></button>`).join('')}</div>${site.hero.brandLayout !== 'watermark' ? `<div class="g-row">${textField('브랜드 한글 문구 · 선택','brandTagline',site.hero.brandTagline,80,'사람을 먼저 생각하는 보험의 기준, 해온')}${textField('브랜드 영문 보조문구 · 선택','brandSubline',site.hero.brandSubline,100,'LIFE INSURANCE FOR A BRIGHTER TOMORROW')}</div>` : ''}</section><details class="g-advanced-note"><summary>더 적을 내용 · 등록번호·주소·지사</summary>${textField('지사명','branch',site.agent.branch,120)}${textField('설계사 등록번호','registration',site.agent.registrationNumber,80)}${textField('사무실 주소','address',site.contact.officeAddress,240)}</details>`;
  } else {
    const stale = lastArchive && revision !== lastArchiveRevision;
    const optionalContactSummary = `${site.contact.email ? `<br>이메일 ${e(site.contact.email)}` : ''}${site.contact.fax ? `<br>팩스 ${e(site.contact.fax)}` : ''}${site.contact.instagramUrl ? `<br>인스타그램 ${e(site.contact.instagramUrl)}` : ''}`;
    panel.innerHTML = `<p class="g-kicker">03 · 확인하고 저장</p><h2 tabindex="-1">내보내기 전에 한 번 더 확인하세요.</h2><dl class="g-summary"><dt>화면 구성</dt><dd>${e(TEMPLATE_META[site.template].name)} · ${e(PALETTES[site.palette || 'navy'].name)} · ${e(BRAND_CHOICES.find(choice => choice.id === (site.hero.brandLayout || 'soft-panel'))?.name || '소프트 브랜드 패널')}</dd><dt>담당자와 연락</dt><dd>${e(site.agent.name || '아직 비어 있음')} · ${e(site.agent.company || '아직 비어 있음')}<br>${e(site.contact.phone || '아직 비어 있음')} · ${e(site.contact.availableHours || '아직 비어 있음')}${optionalContactSummary}</dd><dt>고른 문구</dt><dd>상담 분야 ${site.specialties.length}개 · 상담 과정 ${site.process.length}단계 · 자주 묻는 질문 ${site.faqs.length}개</dd><dt>지금 상태</dt><dd>아직 공개되지 않은 작업본입니다. 저절로 공개되거나 전송되지 않습니다.</dd></dl><section class="g-handoff"><h3>먼저 저장하고, 완성된 화면으로 확인해 보세요.</h3><p>지금까지 입력한 이름·연락처·사진·로고가 그대로 들어간 실제 크기 화면이 열립니다. 고객이 보게 될 모습 그대로입니다.</p><button type="button" class="g-btn g-btn-primary" data-action="save-demo">초안 저장 및 시연</button></section><label class="g-check"><input type="checkbox" data-confirm="scope"${scopeConfirmed ? ' checked' : ''}><span>고른 문구가 실제로 상담하는 범위와 맞는지 확인했습니다.</span></label><label class="g-check"><input type="checkbox" data-confirm="links"${linksConfirmed ? ' checked' : ''}><span>전화번호와 카카오톡·인스타그램 주소가 어디로 연결되는지 확인했습니다.</span></label><label class="g-check"><input type="checkbox" data-confirm="assets"${assetsConfirmed ? ' checked' : ''}><span>사진과 로고를 써도 되는지 확인했습니다.</span></label><section class="g-handoff"><h3>마지막으로 파일로 내보내기</h3><p>세 가지 확인을 마치면 압축 파일(ZIP) 하나가 내려받아집니다. 안에는 인쇄해서 볼 수 있는 화면과, 나중에 이어서 고칠 수 있는 작업 파일이 들어 있습니다.</p><p><strong>내보내도 페이지가 공개되거나 누군가에게 전송되지 않습니다.</strong> 실제로 올리기 전에는 소속 조직의 확인이 필요합니다.</p><p>파일에는 이름·연락처·사진·로고가 들어 있습니다. 전달할 사람에게만 보내 주세요.</p><button type="button" class="g-btn g-btn-primary g-save" data-action="save-zip"${exporting ? ' disabled' : ''}>${exporting ? '파일 만드는 중…' : '파일로 내보내기'}</button></section>${lastArchive ? `<section class="g-result"><p class="g-kicker">파일이 준비되었습니다.</p><h3>내려받기 폴더를 확인해 주세요.</h3><p>압축을 풀지 말고 그대로 전달하시면 됩니다. 안에는 인쇄해서 볼 수 있는 화면과, 나중에 이어서 고칠 수 있는 작업 파일이 들어 있습니다.</p>${stale ? '<p class="g-warning">파일을 만든 뒤에 내용을 고쳤습니다. 지금 내용으로 다시 내보내 주세요.</p>' : ''}<button type="button" class="g-btn" data-action="redownload">다시 내려받기</button></section>` : ''}<p class="g-note">여기의 확인란은 작성자 본인의 점검 기록입니다. 승인 절차를 대신하지 않습니다.</p>`;
  }
}
...
document.addEventListener('input', event => {
  ...
  const setters: Record<string,(value:string)=>void> = {
    ...
    phone: value => { site.contact.phone = value; }, email: value => { site.contact.email = value; }, fax: value => { site.contact.fax = value; }, kakao: value => { site.contact.kakaoUrl = value; }, instagram: value => { site.contact.instagramUrl = value; }, hours: value => { site.contact.availableHours = value; },
    ...
    brandTagline: value => { site.hero.brandTagline = value; }, brandSubline: value => { site.hero.brandSubline = value; },
  };
...
document.addEventListener('change', async event => {
  ...
  try {
    ...
    if (input instanceof HTMLInputElement && input.id === 'guided-file' && input.files?.[0]) {
      ...
      site = resetPublication(imported); purpose = importedProject.editor.purposeId || ''; requestedDomain = importedProject.handoff.requestedDomain || ''; projectSource = 'imported'; changed(); renderPanel(); message('저장해 둔 작업을 불러왔습니다. 문구·사진·로고·구성·색상이 그대로 돌아왔습니다.');
    }
    ...
    if (input instanceof HTMLInputElement && ['guided-logo','guided-logo-mark'].includes(input.id) && input.files?.[0]) {
      const file = input.files[0]; input.value = '';
      if (file.size > MAX_IMAGE_BYTES || !['image/jpeg','image/png','image/webp'].includes(file.type)) throw new Error('JPG·PNG·WebP, 3MB 이하 로고 이미지를 선택해 주세요.');
      const data = await new Promise<string>((resolve,reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(new Error('로고 이미지를 읽지 못했습니다.')); reader.readAsDataURL(file); });
      const invalid = validateImageSource(data); if (invalid) throw new Error(invalid);
      if (input.id === 'guided-logo') {
        site.agent.logoImage = data;
        if (!site.agent.logoMarkImage) site.agent.logoMarkImage = data;
        message('메인 로고를 넣었습니다. 미리보기에도 바로 반영됩니다.');
      } else {
        site.agent.logoMarkImage = data;
        message('상단·푸터용 로고를 넣었습니다. 미리보기에도 바로 반영됩니다.');
      }
      changed('home');
    }
  } catch(error) { showError(error); }
