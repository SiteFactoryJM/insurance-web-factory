...
const patterns: Record<string, { label: string; options: readonly PatternOption[] }> = {
  ...
  footer: { label: "연락처·고지 구성", options: [["classic", "클래식", "왼쪽 소개, 오른쪽 연락처"], ["columns", "세 단의 정보", "소개·소속·연락처를 나란히"], ["minimal", "간결한 정렬", "필수 정보를 순서대로 정리"]] },
...
function writingSection(id: DesignSectionId): string {
  const sections: Record<DesignSectionId, { title: string; key: string; target: string; content: () => string }> = {
    ...
    contact: { title: "전화·오픈채팅 안내", key: "direct-contact", target: "contact", content: () => '<p class="hint">고객용 신청서와 주제 선택 단계는 제공하지 않습니다. 연락처·고지에서 전화번호와 오픈채팅 주소를 수정하면 연락 버튼에 반영됩니다. 편집 미리보기에서는 실제 연결을 실행하지 않습니다.</p>' },
  };
...
function useExampleGroup(key: string): void {
  const labels: Record<string, string> = { profile: "프로필", hero: "첫 화면 문구", intro: "소개 문구", specialties: "상담 분야", process: "진행 과정", faqs: "자주 묻는 질문", reviews: "후기", footer: "연락처·고지", contact: "연락처", compliance: "보험 고지", topics: "상담 주제", brief: "제작 방향" };
...
export function renderPrintBrief(config: SiteConfig): string {
  ...
  const blocks: { title: string; body: string }[] = [
    ...
    { title: "연락처·고지과 연락처", body: `<table>${printRows([["연락처·고지 제목", config.footer?.heading || `${config.agent.name} ${config.agent.title}`], ["추가 안내", config.footer?.note], ["전화", config.contact.phone], ["이메일", config.contact.email], ["상담 시간", config.contact.availableHours], ["주소", config.contact.officeAddress], ["카카오톡", config.contact.kakaoUrl], ["인스타그램", config.contact.instagramUrl], ["지도", config.contact.mapUrl]])}</table>` },
