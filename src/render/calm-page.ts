...
function hero(site: SiteConfig): string {
  ...
  if (pattern === 'editorial') {
    visual = `<div class="hero-visual"><figure class="hero-scene"><img src="${e(site.hero.image || site.agent.profileImage || '/assets/profile-placeholder.svg')}" alt="${site.hero.image ? '' : e(`${site.agent.name || '담당자'} 프로필`)}" width="1536" height="1024" fetchpriority="high">${certificationBadge}</figure>${adviserCard(site)}</div>`;
  } else if (pattern === 'portrait') {
    visual = `<div class="hero-visual"><figure class="portrait-figure">${profile(site, '', true)}${certificationBadge}</figure>${adviserCard(site)}</div>`;
  } else {
    visual = `<div class="hero-visual hero-statement-person">${profile(site, '', true)}${certificationBadge}${adviserCard(site)}</div>`;
  }
...
function about(site: SiteConfig): string {
  ...
  if (pattern === 'profile') {
    ...
    const career = site.sections.career && site.career.length
      ? `<div class="about-profile-value about-profile-career"><strong>경력</strong><span>${site.career.map(e).join('<br>')}</span></div>`
      : '';
    const intro = `<div class="about-profile-content"><div class="about-heading about-profile-heading"><p class="eyebrow">담당자 소개</p><h2 id="about-title">당신의 보험,<br>누구에게 맡기시겠어요?</h2></div><div class="about-copy about-profile-copy"><div class="about-profile-lead"><p>가입은 누구나 시켜줍니다.</p><p>중요한 건 그 다음을 <strong>누가 책임지느냐 입니다.</strong></p></div><div class="about-profile-message"><p>좋은 상품을 고르는 것보다 더 중요한 건,<br>끝까지 곁에서 관리해 줄 사람이 있느냐 입니다.<br><strong>${e(site.agent.name)}이 그 한 사람이 되어 드리겠습니다.</strong></p></div><div class="about-profile-values adviser-principle${career ? ' about-profile-values-with-career' : ''}" aria-label="상담 약속과 경력"><div class="adviser-principle-copy about-profile-principle-copy"><h3>1:1 · 정직 · 동행${career ? ' · 경력' : ''}</h3><p class="adviser-principle-body">전담 평생관리 · 강요 없는 상담 · 보험금 청구까지${career ? ` · ${e(site.career.join(' · '))}` : ''}</p></div><div class="about-profile-value"><strong>1:1</strong><span>전담 평생관리</span></div><div class="about-profile-value"><strong>정직</strong><span>강요 없는 상담</span></div><div class="about-profile-value"><strong>동행</strong><span>보험금 청구까지</span></div>${career}</div>${extraFacts ? `<div class="agent-meta">${extraFacts}</div>` : ''}</div></div>`;
...
function casesTicker(site: SiteConfig): string {
  const items = [
    { title: '내 보험, 제대로 준비되어 있을까요?', body: '현재 가입 중인 보험을 한눈에 보기 쉽게 정리해드립니다. 부족하거나 중복된 보장은 없는지 꼼꼼하게 점검해드립니다.', singleLine: false },
    { title: '보험 리모델링', body: '중증 보장에만 치우치지 않고 경증 질환이나 일상적인 치료 상황에서도 활용할 수 있는 보장이 있는지 꼼꼼하게 점검합니다.', singleLine: true },
    { title: '나에게 맞는 맞춤 설계', body: '나이·직업·가족력·생활환경을 고려해 꼭 필요한 보장을 우선순위에 맞춰 안내해드립니다.', singleLine: true },
    { title: '보험금 청구 지원', body: '진단·수술·사고 시 받을 수 있는 보험금을 놓치지 않도록 도와드립니다. 병원 내원부터 필요한 서류와 청구 절차까지 꼼꼼하게 안내해드립니다.', singleLine: true },
    { title: '우리 가족 보험 통합 관리', body: '부모님·배우자·자녀까지 가족 전체의 보장을 함께 살펴보고, 가입 여부와 관계없이 지속적으로 관리해드립니다.', singleLine: true },
    { title: '은퇴·노후 준비', body: '연금·저축성 상품 등을 활용해 막연한 노후 걱정을 구체적인 준비 계획으로 바꿔드립니다.', singleLine: true },
    { title: '365일 언제든 연락 가능', body: '연중무휴로 언제든지, 늦은 밤이나 새벽에도 편하게 연락 주시면 친절하게 안내해드립니다.', singleLine: true },
  ];
...
function recruitment(site: SiteConfig): string {
  ...
  const contactRows = [
    phone ? `<a class="recruit-contact-link" data-contact-link="phone" href="${e(phone)}"><span class="recruit-contact-icon">${icon('phone', 22)}</span><span class="recruit-contact-label">전화</span><span class="recruit-contact-meta">${e(site.contact.phone)}</span><span class="recruit-contact-chevron">${icon('chevron', 18)}</span></a>` : '',
