...
function about(site: SiteConfig): string {
  ...
  if (pattern === 'profile') {
    ...
    const intro = `<div class="about-profile-content"><div class="about-heading about-profile-heading"><p class="eyebrow">담당자 소개</p><h2 id="about-title">당신의 보험,<br>누구에게 맡기시겠어요?</h2></div><div class="about-copy about-profile-copy"><div class="about-profile-lead"><p>가입은 누구나 시켜줍니다.</p><p>중요한 건 그 다음을 <strong>누가 책임지느냐 입니다.</strong></p></div><div class="about-profile-message"><p>좋은 상품을 고르는 것보다 더 중요한 건,<br>끝까지 곁에서 관리해 줄 사람이 있느냐 입니다.<br><strong>${e(site.agent.name)}이 그 한 사람이 되어 드리겠습니다.</strong></p></div><div class="about-profile-values adviser-principle" aria-label="상담 약속"><div class="adviser-principle-copy about-profile-principle-copy"><h3>1:1 · 정직 · 동행</h3><p class="adviser-principle-body">전담 평생관리 · 강요 없는 상담 · 보험금 청구까지</p></div><div class="about-profile-value"><strong>1:1</strong><span>전담 평생관리</span></div><div class="about-profile-value"><strong>정직</strong><span>강요 없는 상담</span></div><div class="about-profile-value"><strong>동행</strong><span>보험금 청구까지</span></div></div>${site.sections.career && site.career.length ? `<ul class="career-list">${site.career.map(c => `<li>${e(c)}</li>`).join('')}</ul>` : ''}${extraFacts ? `<div class="agent-meta">${extraFacts}</div>` : ''}</div></div>`;
...
function recruitment(site: SiteConfig): string {
  ...
  return `<section class="section recruit-section" id="recruit" aria-labelledby="recruit-title"><div class="container recruit-shell"><header class="recruit-heading"><p class="recruit-kicker">RECRUIT</p><h2 id="recruit-title">함께 성장할 동료를 찾습니다.</h2><p class="recruit-lead">혼자 가면 빨리 가지만, 함께 가면 멀리 갑니다.</p><p class="recruit-support">보험이라는 일을 제대로 배우고 정직하게 오래 하고 싶은 분이라면,<br><strong>${e(agentName)}과 함께 시작해 보세요.</strong></p></header><div class="recruit-panel"><div class="recruit-first"><span class="recruit-number">01</span><div><h3>${e(items[0].title)}</h3><p>${items[0].bodyLines.map(line => e(line)).join('<br>')}</p></div></div><div class="recruit-main"><div class="recruit-fit"><span class="recruit-intro-mark" aria-hidden="true"></span><h3>이런 분과<br>함께하고 싶습니다.</h3><ul>${fitPoints.map(point => `<li><span class="recruit-check" aria-hidden="true">✓</span><span>${e(point)}</span></li>`).join('')}</ul></div><ol class="recruit-list" start="2">${items.slice(1).map((item, index) => `<li><span class="recruit-number">${formatIndex(index + 1)}</span><div><h3>${e(item.title)}</h3><p>${item.bodyLines.map(line => e(line)).join('<br>')}</p></div></li>`).join('')}</ol></div><aside class="recruit-contact" aria-label="리쿠르트 문의"><div class="recruit-contact-person">${profile(site, 'recruit-contact-avatar')}<div><strong>${e(agentName)} <span aria-hidden="true">·</span> ${e(role || '보험설계사')}</strong></div></div><div class="recruit-contact-links">${contactRows}</div><p class="recruit-contact-footer">부담 갖지 말고 편하게 연락주세요.</p></aside></div></div></section>`;
}
