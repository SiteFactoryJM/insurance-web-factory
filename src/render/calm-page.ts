...
function hero(site: SiteConfig): string {
  const copy = `<div class="hero-copy"><p class="eyebrow"><span class="small-line" aria-hidden="true"></span>${e(site.hero.eyebrow || '보험을 이해하는 시간')}</p><h1 id="hero-title">${e(site.hero.headline).replace(/\n/g,'<br>')}</h1><p class="hero-description">${e(site.hero.subheadline)}</p><div class="hero-actions"><a class="button" href="#contact">상담 요청하기 ${arrow}</a><a class="text-link" href="#specialties">상담 분야 살펴보기 <span aria-hidden="true">↓</span></a></div><p class="hero-note">상담 요청은 보험 가입 신청이 아닙니다.</p></div>`;
  const open = (kind: string) => `<section class="hero container ${kind}" id="home" aria-labelledby="hero-title">`;
  switch(site.template) {
    case 'warm-care':
      return `${open('hero-editorial')}${portrait(site)}<div class="editorial-story">${copy}${site.intro.philosophy?`<blockquote class="hero-quote">“${e(site.intro.philosophy)}”</blockquote>`:''}</div></section>`;
    case 'premium-navy':
      return `${open('hero-premium')}${copy}${portrait(site)}<div class="hero-criteria"><p>가입 전 확인할 기준</p><a href="${site.templateContent?.[site.template]?.focus?.length?'#focus':'#specialties'}">01 보장 범위</a><a href="${site.templateContent?.[site.template]?.focus?.length?'#focus':'#specialties'}">02 유지할 예산</a><a href="${site.templateContent?.[site.template]?.focus?.length?'#focus':'#specialties'}">03 계약 조건</a></div></section>`;
    case 'clean-minimal':
      return `${open('hero-report')}${copy}<aside class="report-identity" aria-label="상담 담당자">${portrait(site)}</aside><dl class="report-summary"><div><dt>상담 담당</dt><dd>${e(site.agent.name)}</dd></div><div><dt>상담 가능 시간</dt><dd>${e(site.contact.availableHours)}</dd></div><div><dt>상담 시작</dt><dd>질문 정리부터</dd></div></dl></section>`;
    case 'local-friendly': {
      const topics=site.consultation?.topics || [];
      return `${open('hero-concierge')}<div class="concierge-person">${portrait(site)}<p>${e(site.contact.availableHours)}<br>상담 가능 시간</p></div>${copy}<nav class="quick-topics" aria-label="상담 시작 주제">${topics.slice(0,3).map((topic,i)=>`<a href="#contact" data-start-topic="${e(topic)}"><span class="topic-number">0${i+1}</span><span>${e(topic)}</span>${arrow}</a>`).join('')}</nav></section>`;
    }
    default:
      return `${open('hero-corporate')}${copy}<div class="corporate-profile">${portrait(site)}<p class="profile-caption">${e(site.agent.company)} · 상담 가능 ${e(site.contact.availableHours)}</p></div></section>`;
  }
}
function focus(site: SiteConfig): string {
  const content=site.templateContent?.[site.template];
  if(!content?.focus?.length)return '';
  return `<section class="section container focus-section" id="focus" aria-labelledby="focus-title"><div class="section-heading"><div><p class="eyebrow">상담 전 살펴보기</p><h2 id="focus-title">${e(content.focusTitle || '선택 전에 확인할 내용')}</h2></div></div><div class="focus-grid">${content.focus.map((card,i)=>`<article><span class="section-index">${formatIndex(i)}</span><h3>${e(card.title)}</h3><p>${e(card.body)}</p></article>`).join('')}</div></section>`;
}
...
function process(site: SiteConfig): string {
  ...
  return `<section class="section container" id="process" aria-labelledby="process-title"><div class="section-heading"><div><p class="eyebrow">상담 진행 안내</p><h2 id="process-title">상담은 이 순서로 진행합니다.</h2></div><p>원하는 내용을 확인하고,<br>연락을 통해 일정을 조율합니다.</p></div><ol class="process-grid">${site.process.map((p,i)=>`<li><span class="process-number">${formatIndex(i)}</span><h3>${e(p.title)}</h3><p>${e(p.body)}</p></li>`).join('')}</ol></section>`;
}
...
function faq(site: SiteConfig): string {
  ...
  const expanded=['warm-care','local-friendly'].includes(site.template);
  const body=expanded ? `<div class="faq-cards">${site.faqs.map((f,i)=>`<article><span class="section-index">Q${i+1}</span><h3>${e(f.question)}</h3><p>${e(f.answer)}</p></article>`).join('')}</div>`
    : site.template==='clean-minimal' ? `<dl class="faq-table">${site.faqs.map(f=>`<div><dt>${e(f.question)}</dt><dd>${e(f.answer)}</dd></div>`).join('')}</dl>`
    : `<div class="faq-list">${site.faqs.map(f=>`<details><summary>${e(f.question)}<span aria-hidden="true">+</span></summary><p>${e(f.answer)}</p></details>`).join('')}</div>`;
...
function contact(site: SiteConfig): string {
  return `<section class="section contact-section" id="contact" aria-labelledby="contact-title"><div class="container contact-grid"><div class="contact-copy"><p class="eyebrow">무료 상담 신청 · 샘플</p><h2 id="contact-title">상담을 준비하는<br>네 가지 단계.</h2><p>간단한 내용을 남기는 것부터 시작합니다.<br>마지막 단계에서 다시 확인할 수 있습니다.</p><div class="contact-person"><img src="${e(site.agent.profileImage || '/assets/profile-placeholder.svg')}" alt="" width="72" height="72" loading="lazy"><div><strong>${e(site.agent.name)} 보험설계사</strong><span>${e(site.agent.company)}</span></div></div><p class="support-note">이 페이지는 디자인 검토용입니다.<br>상담료와 실제 취급 범위는 운영 전 확정합니다.</p>${renderSocialLinks(site)}</div>${site.sections.contactForm?renderContactForm(site):'<div class="notice-card"><h3>상담 신청을 준비하고 있습니다.</h3><p>현재 페이지에서는 상담을 접수하지 않습니다.</p></div>'}</div></section>`;
}
export function renderCalmPage(site: SiteConfig): string {
  ...
  return `<main id="main" tabindex="-1">${hero(site)}${trust}${order[site.template].map(key=>modules[key]).join('')}${location}</main>`;
}
