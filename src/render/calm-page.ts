...
function hero(site: SiteConfig): string {
  ...
  if (pattern === 'editorial') {
    visual = `<div class="hero-visual"><figure class="hero-scene"><img src="${e(site.hero.image || '/assets/atelier-interior.webp')}" alt="" width="1536" height="1024" fetchpriority="high"><figcaption>차분히 살펴보고, 충분히 이해하는 시간.</figcaption></figure><div class="hero-adviser">${profile(site, '', true)}<div><p>함께 상담할 사람</p><strong>${e(site.agent.name)}</strong><span>${e(site.agent.company)}</span></div></div></div>`;
