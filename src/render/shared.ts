...
export function renderContactButtons(site: SiteConfig, compact = false): string {
  ...
  return `<div class="direct-contact-actions${compact ? ' contact-compact' : ''}">${phone ? `<a class="button direct-phone" data-contact-link="phone" href="${e(phone)}" aria-label="${e(site.agent.name || '담당자')}에게 전화하기">${phoneIcon}<span>전화하기</span></a>` : ''}${kakao ? `<a class="button direct-kakao" data-contact-link="kakao" href="${e(kakao)}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer" aria-label="카카오톡 오픈채팅 열기 (새 창)">${chatIcon}<span>카카오톡</span></a>` : ''}${!phone && !kakao ? '<p class="support-note">연락처 확인 후 연결 버튼이 표시됩니다.</p>' : ''}</div>`;
}

export function renderHeader(site: SiteConfig): string {
  const hidden = getDesign(site).hiddenSections;
  const links = `${!hidden.includes('services') ? '<a href="#specialties">상담 분야</a>' : ''}${!hidden.includes('about') ? '<a href="#about">담당자 소개</a>' : ''}${site.sections.faq && !hidden.includes('faq') ? '<a href="#faq">자주 묻는 질문</a>' : ''}`;
  const contactTarget = hidden.includes('contact') ? '#footer' : '#contact';
  return `<header class="site-header"><div class="container header-inner"><a class="brand" href="/" aria-label="${e(site.agent.name)} 보험상담 홈">${site.agent.logoImage ? `<img class="brand-logo" src="${e(site.agent.logoImage)}" alt="${e(site.agent.company)} 로고" width="44" height="44">` : ''}<span><strong>${e(site.agent.name || '담당자 이름')} <span class="brand-service">${e(site.agent.title || '보험설계사')}</span></strong><span class="brand-sub">${e(site.agent.company)}</span></span></a><nav class="desktop-nav" aria-label="주요 메뉴">${links}</nav><div class="header-tools"><a class="button header-contact" href="${contactTarget}">연락 방법</a><details class="mobile-menu"><summary>메뉴</summary><nav aria-label="모바일 메뉴">${links}<a href="${contactTarget}">연락 방법</a></nav></details></div></div></header>`;
}
