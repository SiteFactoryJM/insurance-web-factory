...
export function renderInstagramLink(site: SiteConfig): string {
  ...
  return instagram ? `<a class="text-link instagram-link" data-contact-link="instagram" href="${e(instagram)}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer" aria-label="인스타그램 프로필 열기 (새 창)">인스타그램 ${arrow}</a>` : '';
}
