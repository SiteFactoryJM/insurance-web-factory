...
const instagramIcon = '<svg viewBox="0 0 24 24" width="21" height="21" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true" focusable="false"><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg>';
...
export function renderContactButtons(site: SiteConfig, compact = false): string {
  ...
  const instagram = compact ? '' : instagramProfileUrl(site.contact.instagramUrl);
  return `<div class="direct-contact-actions${compact ? ' contact-compact' : ''}">${phone ? `<a class="button direct-phone" data-contact-link="phone" href="${e(phone)}" aria-label="${e(site.agent.name || '담당자')}에게 전화하기">${phoneIcon}<span>${compact ? '전화' : '전화로 문의하기'}</span></a>` : ''}${kakao ? `<a class="button direct-kakao" data-contact-link="kakao" href="${e(kakao)}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer" aria-label="카카오톡 오픈채팅 열기 (새 창)">${chatIcon}<span>${compact ? '오픈채팅' : '카카오톡 오픈채팅'}</span></a>` : ''}${instagram ? `<a class="button direct-instagram" data-contact-link="instagram" href="${e(instagram)}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer" aria-label="인스타그램 프로필 열기 (새 창)">${instagramIcon}<span>인스타그램</span></a>` : ''}${!phone && !kakao && !instagram ? '<p class="support-note">연락처 확인 후 연결 버튼이 표시됩니다.</p>' : ''}</div>`;
}

export function renderInstagramLink(site: SiteConfig): string {
  const instagram = instagramProfileUrl(site.contact.instagramUrl);
  return instagram ? `<a class="button direct-instagram instagram-link" data-contact-link="instagram" href="${e(instagram)}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer" aria-label="인스타그램 프로필 열기 (새 창)">${instagramIcon}<span>인스타그램</span></a>` : '';
}
...
export function renderContactForm(site: SiteConfig): string {
  ...
  const instagram = instagramProfileUrl(site.contact.instagramUrl);
  const compactRecruitContact = Boolean(site.sections.recruitment);
  ...
  const optional = `${email ? `<div><dt>이메일</dt><dd><a data-email-link href="${e(email)}">${e(site.contact.email!)}</a></dd></div>` : ''}${site.contact.fax?.trim() ? `<div><dt>팩스</dt><dd>${e(site.contact.fax)}</dd></div>` : ''}${instagram && !compactRecruitContact ? `<div><dt>인스타그램</dt><dd><a data-instagram-link href="${e(instagram)}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer">프로필 보기 ${arrow}</a></dd></div>` : ''}`;
  const notices = compactRecruitContact ? '' : `<p class="support-note">공개 대화방에는 주민등록번호·병력·계약서 원본을 남기지 마세요.</p>${site.demo?.enabled ? '<p class="demo-contact-notice">디자인 예시입니다. 이 페이지의 연락 버튼은 표시된 담당자에게 실제로 연결됩니다.</p>' : ''}`;
  return `<div class="direct-contact-panel" data-direct-contact><p class="eyebrow">담당자에게 직접 문의</p><h3>편한 채널로 바로 연락하세요.</h3>${renderContactButtons(site)}<dl class="direct-contact-details">${identity}<div><dt>전화번호</dt><dd>${e(site.contact.phone || '미입력')}</dd></div><div><dt>상담 시간</dt><dd>${e(site.contact.availableHours || '담당자에게 확인')}</dd></div>${optional}</dl>${notices}</div>`;
}
export function renderFooter(site: SiteConfig): string {
  const pattern = getDesign(site).footer;
  const footerLogo = site.agent.logoMarkImage || site.agent.logoImage;
  const email = directEmailHref(site.contact.email);
  const instagram = instagramProfileUrl(site.contact.instagramUrl);
  const copyrightMonth = new Date(Date.now() + 9 * 60 * 60 * 1000).toISOString().slice(0, 7);
  const compactRecruitFooter = Boolean(site.sections.recruitment);
  ...
  const note = !compactRecruitFooter && site.footer?.note ? `<p class="footer-note">${e(site.footer.note)}</p>` : '';
  return `<footer class="site-footer footer-${pattern}" id="footer" data-pattern="footer-${pattern}"><div class="container"><div class="footer-top"><div class="footer-identity"><h2>${e(site.footer?.heading || `${site.agent.name || '담당자'} 보험상담`)}</h2>${pattern === 'columns' ? note : business + note}</div>${pattern === 'columns' ? `<div class="footer-business">${business}</div>` : ''}<div class="footer-contact"><p>${e(site.contact.phone)}${email ? `<br><a data-email-link href="${e(email)}">${e(site.contact.email!)}</a>` : ''}${site.contact.fax?.trim() ? `<br>팩스 ${e(site.contact.fax)}` : ''}${instagram && !compactRecruitFooter && !site.footer?.hideInstagram ? `<br><a data-instagram-link href="${e(instagram)}" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer">인스타그램 ${arrow}</a>` : ''}<br>상담 시간 ${e(site.contact.availableHours)}</p></div></div><div class="footer-legal"><p>${e(site.compliance.footerDisclaimer)}</p>${site.compliance.advertisingReviewNumber ? `<p>${e(site.compliance.advertisingReviewNumber)}${site.compliance.advertisingReviewExpiresAt ? ` · 유효기간 ${e(site.compliance.advertisingReviewExpiresAt)}` : ''}</p>` : ''}<p>상담은 가입 신청과 별개입니다. 계약 전 상품설명서와 약관을 읽고 최종 결정을 해 주세요.</p><div class="footer-bottom"><p>© ${copyrightMonth} ${e(site.agent.name)}</p><div class="footer-brand-end">${footerLogo ? `<img class="footer-brand-logo" src="${e(footerLogo)}" alt="${e(site.agent.company || '소속')} 로고" width="140" height="140">` : ''}</div></div></div></div></footer>`;
}
