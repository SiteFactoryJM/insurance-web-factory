...
export function renderContactForm(site: SiteConfig): string {
  ...
  const goals = ['현재 보험 점검', '새 보험 준비', '보험 용어 이해', '상담하며 정할게요'];
  return `<form class="consultation-form" data-contact-form data-submission-mode="discard" method="post" action="/#contact" novalidate autocomplete="off">
    ...
    <ol class="stepper" aria-label="상담 신청 단계">${['분야 선택','상담 목적','예시 정보','최종 확인'].map((s,i)=>`<li data-step-indicator${i===0?' aria-current="step"':''}><span aria-hidden="true">${i+1}</span><span>${s}</span></li>`).join('')}</ol>
    <p class="step-status" data-step-status aria-live="polite">1 / 4단계 · 분야 선택</p>
    <fieldset data-step="0" disabled><legend><span tabindex="-1" data-step-title>어떤 분야가 궁금하세요?</span></legend><p class="field-help">가장 가까운 분야 하나를 골라 주세요.</p><div class="topic-grid">${topics.map((topic,i)=>`<label class="topic-option"><input type="radio" name="topic" value="${e(topic)}"${i===0?' required':''}><span>${e(topic)}</span></label>`).join('')}</div></fieldset>
    <fieldset data-step="1" hidden disabled><legend><span tabindex="-1" data-step-title>어떤 도움이 필요하세요?</span></legend><p class="field-help">${responsiveCopy('상담 목적을 골라 주세요. 아직 모르겠다면 기본 선택으로 넘어가도 괜찮습니다.', '상담 목적을 골라 주세요. 몰라도 괜찮아요.')}</p><div class="topic-grid" role="group" aria-label="상담 목적 (선택)">${goals.map((goal,i)=>`<label class="topic-option"><input type="radio" name="goal" value="${e(goal)}"${i===goals.length-1?' checked':''}><span>${e(goal)}</span></label>`).join('')}</div><label for="consult-message">덧붙일 내용 <span class="optional">(선택 · 200자 이내)</span></label><p class="field-help" id="message-help">${responsiveCopy('한두 문장이면 충분합니다. 이 항목은 건너뛰어도 괜찮습니다.', '한 문장만 적어도 돼요. 건너뛰기도 가능해요.')}<br>주민등록번호, 병력 등 민감한 정보는 적지 마세요.</p><textarea id="consult-message" name="message" rows="3" maxlength="400" data-max-characters="200" aria-describedby="message-help message-count" placeholder="예: 가입한 보험을 이해하고 싶어요."></textarea><p id="message-count" class="character-count" data-character-count>0 / 200자</p></fieldset>
    <fieldset data-step="2" hidden disabled><legend><span tabindex="-1" data-step-title>예시 연락 정보를 적어주세요.</span></legend><p class="field-help">화면 확인용입니다. 실제 연락은 드리지 않습니다.</p><div class="form-row"><div><label for="consult-name">이름 또는 호칭 <span class="optional">(선택)</span></label><input id="consult-name" name="name" type="text" maxlength="30" placeholder="예: 데모 고객"></div><div><label for="consult-phone">예시 휴대전화 번호 <span class="required">(필수)</span></label><input id="consult-phone" name="phone" type="tel" inputmode="tel" maxlength="13" required aria-describedby="phone-help" placeholder="010-0000-0000"><p id="phone-help" class="field-help">010-0000-0000으로 체험해 보세요.</p></div></div>
