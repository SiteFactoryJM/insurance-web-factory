...
import { escapeHtml as e, jsonForHtml } from '../utils/html.js';
import { instagramProfileUrl } from '../utils/contact-links.js';
...
export function renderPrivacyPage(site: SiteConfig, request: Request): string {
  return `<!doctype html><html lang="ko"><head>${renderHead(site,request)}${pageStyles()}</head><body class="premium-page policy-body font-pretendard" style="${paletteStyle(paletteId(site))};${e(headingFontStyle(site.headingFont))}"><main class="privacy-page" id="main"><div class="container"><header class="simple-header"><a href="/">홈페이지로 돌아가기</a><h1>개인정보·외부 연결 안내</h1><p>고객용 상담 입력란이나 온라인 접수 기능을 제공하지 않습니다.</p></header><article class="policy-card"><h2>1. 전화·카카오톡·인스타그램 연결</h2><p>전화 버튼은 기기의 전화 앱으로, 카카오톡 버튼은 설정된 오픈채팅 주소로 연결됩니다. 인스타그램 링크를 설정한 경우 해당 프로필이 새 창에서 열립니다. 이 페이지가 메시지를 작성하거나 자동 발송하지 않습니다.</p><h2>2. 이 페이지의 상담 정보 처리</h2><p>고객의 이름·전화번호·상담 내용을 입력받아 저장하거나 전송하는 기능이 없습니다. 서비스 제공 과정의 접속 기록 등은 실제 호스팅 운영 정책에 따라 별도로 확인해야 합니다.</p><h2>3. 외부 서비스 이용</h2><p>전화나 카카오톡으로 이동한 뒤 제공하는 정보에는 해당 서비스와 상담 담당자의 처리 기준이 적용됩니다. 공개 대화방에는 주민등록번호·병력·계약서 원본을 남기지 마세요.</p>${site.demo?.enabled ? '<h2>4. 제작 화면의 임시 보관</h2><p>페이지를 만드는 화면은 만든 사람이 직접 켰을 때에만 그 브라우저에 작업 내용을 임시로 보관합니다. 보관한 내용에는 입력한 이름·연락처·사진이 들어갈 수 있고, 같은 화면에서 지울 수 있습니다. 파일로 내보내기는 본인 기기로 내려받는 동작입니다.</p>' : ''}<h2>문의 담당</h2><p>${e(site.compliance.privacyOfficer)}</p><p>실제 게시 전에는 운영 주체의 개인정보 안내와 외부 서비스 이용 기준을 확인해야 합니다.</p></article></div></main>${renderFooter(site)}</body></html>`;
}
