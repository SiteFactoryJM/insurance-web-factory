...
const pageStyles = () => `<style>${styles}${premiumStyles}${directContactStyles}</style>`;
...
function renderTemplatePreview(id: TemplateId, site: SiteConfig): string {
  ...
  return `<div class="template-preview preview-${id}" aria-hidden="true"><div class="preview-words"><p>${e(TEMPLATE_META[id].purpose)}</p><strong>${e(headline).replace(/\n/g,'<br>')}</strong><span></span></div><img src="${e(site.agent.profileImage || '/assets/profile-placeholder.svg')}" alt="" loading="lazy" width="240" height="320"><div class="preview-modules"><i></i><i></i><i></i></div></div><p class="palette-caption">구성 안내도 · 실제 페이지는 아래 버튼에서 확인</p><p class="palette-caption">${order.map(key => labels[key]).join(' → ')}</p>`;
}
export function renderTemplateGallery(rawSite: SiteConfig, request: Request): string {
  ...
  return `<!doctype html><html lang="ko"><head>${renderHead(site,request)}${pageStyles()}</head><body data-design-version="${DESIGN_VERSION}" class="premium-page gallery-page font-${site.headingFont}" style="${paletteStyle(paletteId(site))};${e(headingFontStyle(site.headingFont))}"><main class="container" id="main"><header class="simple-header gallery-header"><a href="/proposal">← 조직 도입 안내</a><p class="eyebrow">ATELIER · 목적별 디자인 비교</p><h1>어떤 정보를<br>먼저 보여줄까요?</h1><p>색상만 고르지 말고 고객에게 먼저 설명할 내용을 골라 보세요. 5가지 배치와 6가지 색상을 조합하고, 선택형 제작에서 문구를 고를 수 있습니다.</p><p><a class="button" href="/studio">문구 골라 만들기 ↗</a></p></header><form class="palette-picker" action="/templates" method="get"><fieldset><legend>샘플 색상 고르기</legend><div class="palette-options"><button class="palette-choice" type="submit" name="palette" value="recommended" aria-pressed="${!selected}">배치별 추천 색상</button>${colors}</div></fieldset></form><section class="template-gallery" aria-label="배치 샘플 5가지">${cards}</section></main>${renderFooter(site)}</body></html>`;
}
export function renderPrivacyPage(site: SiteConfig, request: Request): string {
  return `<!doctype html><html lang="ko"><head>${renderHead(site,request)}${pageStyles()}</head><body class="premium-page policy-body font-pretendard" style="${paletteStyle(paletteId(site))};${e(headingFontStyle(site.headingFont))}"><main class="privacy-page" id="main"><div class="container"><header class="simple-header"><a href="/">홈페이지로 돌아가기</a><h1>개인정보·외부 연결 안내</h1><p>고객용 상담 입력란이나 온라인 접수 기능을 제공하지 않습니다.</p></header><article class="policy-card"><h2>1. 전화 및 오픈채팅 연결</h2><p>전화 버튼은 기기의 전화 앱으로, 카카오톡 버튼은 설정된 오픈채팅 주소로 연결됩니다. 이 페이지가 메시지를 작성하거나 자동 발송하지 않습니다.</p><h2>2. 이 페이지의 상담 정보 처리</h2><p>고객의 이름·전화번호·상담 내용을 입력받아 저장하거나 전송하는 기능이 없습니다. 서비스 제공 과정의 접속 기록 등은 실제 호스팅 운영 정책에 따라 별도로 확인해야 합니다.</p><h2>3. 외부 서비스 이용</h2><p>전화나 카카오톡으로 이동한 뒤 제공하는 정보에는 해당 서비스와 상담 담당자의 처리 기준이 적용됩니다. 공개 대화방에는 주민등록번호·병력·계약서 원본을 남기지 마세요.</p>${site.demo?.enabled ? '<h2>4. 제작 도구의 초안 보관</h2><p>선택형 제작 도구는 제작자의 명시적 선택이 있을 때에만 해당 브라우저에 초안을 보관합니다. 초안에는 제작자가 입력한 프로필·연락처·사진이 포함될 수 있으며, 도구에서 삭제할 수 있습니다. 제작 파일 저장은 사용자의 기기로 내려받는 동작입니다.</p>' : ''}<h2>문의 담당</h2><p>${e(site.compliance.privacyOfficer)}</p><p>실제 게시 전에는 운영 주체의 개인정보 안내와 외부 서비스 이용 기준을 확인해야 합니다.</p></article></div></main>${renderFooter(site)}</body></html>`;
}
